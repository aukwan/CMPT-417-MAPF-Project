...
def detect_collision(path1, path2):
    ...
    for t in range(max(len(path1), len(path2))):
        if get_location(path1, t) == get_location(path2, t):
            collison = {'loc': [get_location(path1, t)], 'timestep': t}
            return collison
        elif t > 0 and get_location(path1, t - 1) == get_location(path2, t) and get_location(path1, t) == get_location(path2, t - 1):
            collison = {'loc': [get_location(path2, t - 1), get_location(path2, t)], 'timestep': t}
            return collison
...
def detect_collisions(paths):
    ...
    for i in range(len(paths) - 1):
        for j in range(i + 1, len(paths)):
            first_collison = detect_collision(paths[i], paths[j])
            if first_collison != []:
                collisions.append({'a1': i, 'a2': j, 'loc': first_collison['loc'], 'timestep': first_collison['timestep']})
    return collisions
            ...
            for constraint in constraints:
                ...
                if path is not None:
                    ...
                    if constraint['positive']:
                        violate_agent_ids = paths_violate_constraint(constraint, child['paths'])
                        for i in violate_agent_ids:
                            child['constraints'].append({'agent': i, 'loc': constraint['loc'], 'timestep': constraint['timestep'], 'positive': False})
